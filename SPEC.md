...
@Model class ScheduleItem {
    ...
    var rating: Int?          // 集中度評価（1〜5、未評価時は nil）
...
- 右上のタイマーボタン（`timer` アイコン）→ タイマー画面を全画面シートで表示
...
- 30分ごとに水平 divider を表示（詳細は「時間軸」セクション参照）
...
- ボックスの上部には 2分分の余白を設ける（各ボックスが視覚的に分離して見えるよう）  
  → 例: 14:00 開始のボックスは上端に余白があり 14:02 から始まるように見える。終了時刻は揃える
- 背景色: 紐づくタスクの `colorHex`（休憩ボックスはパステルグレー `#D0D0D0` で固定表示）
- 同一時間帯に複数のタイムボックスが重なる場合は横並びで表示（スケジュール単位でカラム分割）
  - `GeometryReader` で親の幅を取得し、カラム数で均等分割して配置
- タイムボックスをタップ → 親スケジュールのタイムボックス詳細シートを表示

**時間軸（ディバイダー）**
- 30分単位で水平 divider を表示
  - 毎時 :00 ライン: 通常の実線 Divider（`secondary` カラー）
  - 毎時 :30 ライン: 薄い Divider（`opacity(0.4)`、ラベルも薄め）

**初期スクロール位置**
- 画面を開いたとき、現在時刻の約1時間前が画面上部に来るようにスクロールする（`ScrollViewReader` + `onAppear` で実装）
...
- デフォルトの1時間あたりの縦幅: **150pt**（`DayScheduleViewModel.defaultHourHeight`）
...
**表示形式**
- `.presentationDetents([.large])` で全画面に近い高さで表示（削除ボタンまで確実に表示される）
- `.presentationBackground(.regularMaterial)` で背景の透明度を抑えた表示
- `.presentationBackgroundInteraction(.disabled)` でシートタップ時のハイライト演出を無効化

**ツールバー**
- 右上にタイマーアイコン（`timer`）を表示
- タップ → シートを閉じずに `fullScreenCover` でタイマー画面を前面に表示（背景が透明にならないようシート内部から直接表示）
...
- **セグメント一覧**: `timeBoxSegments` の各セグメントを縦並びで表示
  ...
  - スケジュールの終了時刻（`endDateTime`）を過ぎている場合は、タイマー状態によらず全セグメントを「実行済み」で表示する

**集中度評価（5段階）**
- セグメント一覧の下・編集/削除ボタンの上に表示
- タイトル「このタスクの集中度を評価しよう」
- 1〜5点の横並びボタン（選択で `ScheduleItem.rating` に保存）
...
| 2点 | オレンジ（`.orange`） |
| 3点 | 黄色（`.yellow`） |
| 4点 | 黄緑（`Color(red: 0.6, green: 0.8, blue: 0.2)`） |
| 5点 | 緑（`.green`） |

- 選択済みのボタンは背景色を不透明にして強調表示、未選択は薄い背景色
- 選択済みのボタンをもう一度タップすると未選択状態（`nil`）に戻る
- 評価は `ScheduleItem.rating: Int?` として SwiftData に永続保存される

**ボタン**
- 「編集」ボタン: スケジュール追加と同じ UI の編集画面を表示
- 「削除」ボタン: スケジュールを削除し、関連する通知もキャンセルする。削除対象のスケジュールが現在実行中のタイマーに紐づいている場合は、タイマー・音楽も同時に停止する
...
| 責務 | 内容 |
|------|------|
| スケジュール監視 | 10秒ごとに SwiftData を参照し、開始時刻が現在時刻内のスケジュールを検出して自動スタート。また、scheduleSynced で動作中のタイマーが参照するスケジュールが削除されていた場合は自動キャンセル（孤立タイマーの除去） |
| タイマー制御 | カウントダウン（1秒 Timer）、一時停止・再開・キャンセル |
| フェーズ管理 | 作業→休憩→作業… の自動切り替え |
| バックグラウンド対応 | アプリがバックに移ったとき背景通知をスケジュール登録し、復帰時に経過時間を再計算 |
| 状態永続化 | `UserDefaults` にタイマー状態を保存し、アプリ再起動後も復元。復元時に対応スケジュールが SwiftData に存在するか確認し、削除済みの場合は復元しない |
...
| タイマー | タイマー画面（全画面シート） |
| 統計 | 「集中度の統計を日別とタスク別で評価できる統計データ機能を開発中です。」アラートを表示（機能開発中のプレースホルダー） |
...
|------------|--------|
| スケジュール追加 | `.blue` |
| タイマー | パステル黄緑（`Color(red: 0.6, green: 0.88, blue: 0.6)`） |
| 統計 | `.purple` |
